...
function chainsing1(M::Model, n::Int)
    ...
    for j in 1:m
        ...
        @constraint(
            M,
            [0.5, s[j], x[i] + 10 * x[i+1]] in MOI.RotatedSecondOrderCone(3)
        )
        # t[j] >= 5*(x[i+2] - x[i+3])^2
        @constraint(
            M,
            [0.5, t[j], sqrt(5.0) * (x[i+2] - x[i+3])] in
            MOI.RotatedSecondOrderCone(3)
        )
        # r[j] >= (x[i+1] - 2*x[i+2])^2
        @constraint(
            M,
            [0.5, r[j], x[i+1] - 2.0 * x[i+2]] in MOI.RotatedSecondOrderCone(3)
        )
        # 1/10 * u[j] >= (x[i] - 10*x[i+3])^2
        @constraint(
            M,
            [0.5 * 10^(-1.0), u[j], x[i] - 10.0 * x[i+3]] in
            MOI.RotatedSecondOrderCone(3)
        )
        # p[j] >= r[j]^2
...
function chainsing2(M::Model, n::Int)
    ...
    @constraint(
        ...
        ) in MOI.RotatedSecondOrderCone(2 * m + 2)
    )
    for j in 1:m
        i = ((j - 1) << 1) + 1
        ...
        @constraint(
            M,
            [0.5 * 10.0^(-1.0), u[j], x[i] - 10 * x[i+3]] in
            MOI.RotatedSecondOrderCone(3)
        )
        # p[j] >= r[j]^2
        @constraint(M, [0.5, p[j], r[j]] in MOI.RotatedSecondOrderCone(3))
...
function chainsing3(M::Model, n::Int)
    ...
    @constraint(
        ...
        ) in MOI.RotatedSecondOrderCone(4 * m + 2)
    )
    for j in 1:m
        i = ((j - 1) << 1) + 1
    ...
    for a in argv
        ...
        if m == nothing
            ...
            elseif key == "help"
                ...
                println(
                    "  n The problem scale, the default being 8192. Must be even.",
                )
                println("Options:")
                println("\t--method=[1|2|3] Which formulation to use")
                println("\t--timefile=filename Which formulation to use")
                println(
                    "\t--backend=[generic|mosek|mock] Which backend to use.",
                )
                println(
                    "\t\tgeneric Use generic backend, then copy to MOSEK optimizer",
                )
                println("\t\tmosek   Use mosek as backend")
                println(
                    "\t\tmock    Use generic backend, then copy to JuMP mock optimizer",
                )
